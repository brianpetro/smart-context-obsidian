...
export class ReleaseNotesView extends SmartItemView {
  static get view_type() { return 'smart-context-release-notes-view'; }
  ...
  async render() {
    ...
    // force external links to open in browser
  ...
  get version() {
    const version =
      this.leaf.viewState?.state?.version ??
      this.app.plugins.getPlugin('smart-context')?.manifest.version ??
      '';
    return version;
  }
